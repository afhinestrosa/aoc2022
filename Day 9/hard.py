...
def calculate_tail_path(head_steps):
    ...
    tail_pos = [np.array([0, 0])] * 9
    tail_steps.append(tuple(tail_pos[-1]))
    for s in head_steps:
        ns = int(np.sqrt(np.sum(s**2))) # num of unit steps
        us = s // ns                     # unit step
        for n in range(ns):
            head_pos += us
            tail_pos[0] = process_tail(head_pos, tail_pos[0])
            for m in range(1, len(tail_pos)):
                tail_pos[m] = process_tail(tail_pos[m-1], tail_pos[m])
            tail_steps.append(tuple(tail_pos[-1]))
    return tail_steps
